...
from var_control.raps_utils import validate, get_logits_targets, sort_sum
import pdb
...
def pick_kreg(paramtune_logits, alpha):
    gt_locs_kstar = np.array(
        [
            np.where(np.argsort(x[0]).flip(dims=(0,)) == x[1])[0].item()
...
def pick_parameters(
    ...
    if kreg == None:
        kreg = pick_kreg(paramtune_logits.dataset, alpha)
    if lamda == None:
