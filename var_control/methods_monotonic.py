...
class Ltt:
    @staticmethod
    def fit_target_var(
        metric: Tensor, z_test: Tensor, target: float, delta: float, thresholds: list
    ) -> Tensor:
        ...
        lambda_hat = thresholds[pvals < delta / thresholds.shape[0]]
        # select smallest threshold that passes test
        if lambda_hat.shape[0] == 0:
            predictions = torch.zeros_like(z_test)
            best_lambda = None
        else:
            best_lambda = lambda_hat[-1]
            best_hypothesis = hypothesis_of_threshold(best_lambda)
            # compute predictions set
            predictions = best_hypothesis(z_test)

        return predictions, best_lambda
